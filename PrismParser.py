...
class PrismParser:
    ...
    def call_prism(self, query, args=""):
        # result = subprocess.run([self.path, self.model, '-pf', 'Pmax=? [F "positive"]'], capture_output = True, text = True)
        result = subprocess.run([self.path, self.model, '-pf', query], capture_output = True, text = True)
        ...
        return PrismResult(result_value, model_checking_time=model_checking_time, model_construction_time=model_construction_time).df()