...
from aalpy.utils import save_automaton_to_file, mdp_2_prism_format
...
class LogParser:
    ...
    def build_benchmark(self, prism_name = None):
        ...
        if prism_name:
            mdp_2_prism_format(mdp=model, name='model', output_path=prism_name)
        model = convert_utils.mdp_to_nx(model, self.get_actors())
    
        return model
    
    def get_data_trace(self, path_length, number_paths):
        if not self.data_environment:
            self.build_data()
        paths = random.sample([t for t in self.data_environment if len(t) >= path_length+1], number_paths) # +1 as s_0 does not count
        return paths
